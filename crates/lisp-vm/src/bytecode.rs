use lisp_core::helpers::is_truthy;
use lisp_core::types::{Env, EvalState, LispVal, Op, BinOp, Ty, CompiledLambda};
        ...
        match expr {
            ...
            LispVal::Sym(name) => {
                ...
                if let Some(slot) = self.slot_of(name) {
                    ...
                    self.code.push(Op::LoadCaptured(idx));
                    true
                } else if lisp_core::helpers::is_builtin_name(name) {
                    self.code.push(Op::PushBuiltin(name.to_string()));
                    self.last_result_i64 = false;
                    self.last_result_f64 = false;
                    true
                } else if let Some(ctor) = lisp_core::helpers::lookup_constructor(name) {
                    ...
                    match op.as_str() {
                        ...
                        _ => {
                            ...
                            if let Some(idx) = self.captured_idx(op) {
                                ...
                                if let Some(ctor) = lisp_core::helpers::lookup_constructor(op) {
...
pub fn eval_builtin(
    ...
    match name {
        ...
        "equal?" => {
            ...
            Ok(LispVal::Bool(lisp_core::helpers::lisp_equal(a, b)))
        }
        "not" => {
        ...
        "elapsed" => match args.get(0) {
            Some(v) => {
                let since = lisp_core::helpers::as_float(v).unwrap_or(0.0);
        ...
        "float" => match args.get(0) {
            Some(LispVal::Num(n)) => Ok(LispVal::Float(*n as f64)),
            Some(v) => Ok(lisp_core::helpers::as_float(v)
                .map(LispVal::Float)
        ...
        "boolean" => Ok(LispVal::Bool(lisp_core::helpers::is_truthy(
            args.get(0).unwrap_or(&LispVal::Nil),
        ))),
        // --- Error ---
        "error" => {
        ...
        "doc" => {
            ...
            match lisp_core::helpers::get_doc(&name) {
        ...
        _ => {
            ...
            if name == "load-file" {
                if let (Some(e), Some(s)) = (env, state) {
                    ...
                    let forms = lisp_core::parser::parse_all(&code)
...
pub fn run_lambda_test(cl: &CompiledLambda, args: &[LispVal]) -> Result<LispVal, String> {
    run_compiled_lambda(cl, args, &mut lisp_core::types::Env::new(), &mut lisp_core::types::EvalState::new())
}
