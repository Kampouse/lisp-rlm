...
#[cfg(feature = "llm")]
fn clone_llm_provider(provider: &Option<Box<dyn std::any::Any + Send + Sync>>) -> Option<Box<dyn crate::eval::llm_provider::LlmProvider>> {
    provider.as_ref().and_then(|p| {
        p.downcast_ref::<crate::eval::llm_provider::GenericProvider>()
            .map(|v| crate::eval::llm_provider::LlmProvider::box_clone(v))
    })
}

use lisp_core::helpers::*;
#[cfg(feature = "llm")] use crate::eval::llm_provider::LlmProvider;
...
pub fn handle(
    ...
    match name {
        ...
        "par-map" => {
            #[cfg(all(feature = "llm", not(target_arch = "wasm32")))]
            {
                ...
                let provider = clone_llm_provider(&state.llm_provider);
                ...
                let results: Result<Vec<LispVal>, String> = rt.block_on(async {
                    ...
                    for elem in lst {
                        ...
                        let mut task_state = lisp_core::types::EvalState::fork_for_parallel(
                            &state,
                            provider.as_ref().map(|p| Box::new(p.box_clone()) as Box<dyn std::any::Any + Send + Sync>),
                        );
            ...
            #[cfg(any(target_arch = "wasm32", not(feature = "llm")))]
            {
                // Fall back to sequential map on WASM / no LLM
                let func = args.first().ok_or("par-map: need (f list)")?;
                let lst = match args.get(1) {
                    Some(LispVal::List(l)) => l.clone(),
                    Some(LispVal::Nil) => return Ok(Some(LispVal::List(vec![]))),
        ...
        "par-filter" => {
            ...
            #[cfg(any(target_arch = "wasm32", not(feature = "llm")))]
            {
                // Fall back to sequential filter on WASM / no LLM
                let func = args.first().ok_or("par-filter: need (pred list)")?;
                let lst = match args.get(1) {
                    Some(LispVal::List(l)) => l.clone(),
                    Some(LispVal::Nil) => return Ok(Some(LispVal::List(vec![]))),
            ...
            let matches = if args.len() >= 2 {
                ...
                if let LispVal::Str(spec) = &args[1] {
                    ...
                    if parts.len() == 2 {
                        ...
                        if let Some(ctor) = lisp_core::helpers::lookup_constructor(vn) {
