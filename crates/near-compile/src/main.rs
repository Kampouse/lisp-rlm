...
fn do_build_target_with_config(
    ...
    if target == "near" {
        // schnorr_verify_bip340 / sha256_hash imports are stitched by the
        // EMITTER from the embedded schnorr.wasm (built from schnorr/ — see
        // `make schnorr-wasm`); the old post-hoc python stitcher
        // (k256-schnorr-wasm + wasm_stitch.py, machine-local) was removed
        // 2026-09-08: nothing in the language emits its remaining target
        // (schnorr_verify_clearmsig), so it was unreachable dead weight.
