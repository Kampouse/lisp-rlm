(define (run)
  (let* (
    ...
    ;; Fetch Burrow account FIRST (before any str-cat)
    (args (str-cat "{\"account_id\":\"" acct "\"}"))
    (account-raw (outlayer/view "contract.main.burrow.near" "get_account" args))
    (has-account (if (nil? account-raw) 0 1))
    )
    ;; If no account, return early
    (if (= has-account 0)
        (str-cat "{\"account\":\"" acct "\",\"error\":\"no_burrow_account\",\"prices\":{\"usdt\":\"1\"}}")
        ;; Else continue with prices
        (let* (
