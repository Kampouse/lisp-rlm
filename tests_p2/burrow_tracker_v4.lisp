;; Burrow Tracker v4 — simplified, using Space patterns.
;; Users deposit NEAR for margin tracking credits.
;;
;; Keys:
;;   "owner" — contract owner
;;   "poll/" + account — poll authorization  
  ...
  (let ((inp (near/input)))
    (let ((acct-len (bytes-to-u32 (str-slice inp 0 4))))
      (let ((acct (str-slice inp 4 (+ 4 acct-len))))
        (if (near/has_key (str-cat "u/" acct))
          (let ((dep-ptr (near/load_u128 (str-cat "u/" acct))))
            (near/return_str (to-string dep-ptr)))
          (near/return_str "0"))))))
