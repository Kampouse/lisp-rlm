...
fn run(code: &str, env: &mut Env) -> String {
...
fn norvig_lis_tests() {
    ...
    {
        let r = run("(quote (testing 1 (2.0) -3.14e159))", &mut env);
    ...
    {
        let r = run("(if (> 6 5) (+ 1 1) (+ 2 2))", &mut env);
        check("if_true", &r, "2");
    ...
    // ---- Test 7: begin / set! ------------------------------------------
    {
        let r = run("(begin (define y 1) (set! y (+ y 1)) (+ y 1))", &mut env);
        check("begin_set!", &r, "3");
    ...
    {
        let r = run("((lambda (x) (+ x x)) 5)", &mut env);
    ...
    {
        let r = run("(define twice (lambda (x) (* 2 x))) (twice 5)", &mut env);
        check("twice_5", &r, "10");
        if r == "10" { local_pass += 1; } else { local_fail += 1; }
    }
    ...
    // ---- Test 10: compose (higher-order) -------------------------------
    {
        ...
        let r = run("((compose list twice) 5)", &mut env);
    ...
    {
        run("(define repeat (lambda (f) (compose f f)))", &mut env);
        // (repeat twice) = compose(twice, twice) = λx. twice(twice(x))
        // twice(5)=10, twice(10)=20
        let r1 = run("((repeat twice) 5)", &mut env);
        ...
        let r2 = run("((repeat (repeat twice)) 5)", &mut env);
    ...
    // simple numbers that survive re-evaluation.
    {
        ...
        let r = run("(list (abs -3) (abs 0) (abs 3))", &mut env);
    ...
    // Builtins (append, cons) are now proper first-class values via dispatch_call_with_args.
    {
        ...
        run("(define zip (combine cons))", &mut env);
        let r2 = run("(zip (list 1 2 3) (list 4 5 6))", &mut env);
    ...
    // Adapted from Norvig: uses top-level take/drop + combine _append.
    {
        ...
        let r = run("(riff-shuffle (list 1 2 3 4 5 6))", &mut env);
