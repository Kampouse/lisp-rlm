...
fn test_defmacro_basic() {
    ...
    let r = eval(r#"
        ...
        (double 3)
    "#);
    assert_eq!(r, "6", "basic defmacro should expand (+ 3 3) = 6, got: {}", r);
}
...
fn test_macro_args_not_evaluated() {
    ...
    let r = eval(r#"
        ...
        (identity (+ 1 2))
    "#);
    assert_eq!(r, "3", "identity macro returns unevaluated form which then gets eval'd");
}
...
fn test_macro_receives_list_form() {
    ...
    assert_eq!(r, "(3)", "wrap-list should pass form unevaluated then eval result");
}
...
fn test_unquote_splicing_basic() {
    ...
    "#);
    assert_eq!(r, "(0 1 2 3 4)", "unquote-splicing should flatten list into parent");
}
...
fn test_macro_with_local_helper() {
    ...
    let r = eval(r#"
        ...
        (double-it 5)
    "#);
    assert_eq!(r, "10", "macro should be able to call helper from defining scope");
}
...
fn test_macro_chain() {
    let r = eval(r#"
        ...
        (add2 10)
    "#);
    assert_eq!(r, "12", "chained macros: add2 should expand to add1(add1(10))");
}
...
fn test_macro_type_check_false() {
    ...
    "#);
    assert_eq!(r, "(false false false)", "macro? should be false for non-macros");
}
...
fn test_defmacro_needs_symbol() {
    ...
    assert!(r.starts_with("ERROR"), "defmacro with non-symbol name should error, got: {}", r);
}

#[test]
fn test_defmacro_needs_params() {
    let r = eval("(defmacro mymac)");
    assert!(r.starts_with("ERROR"), "defmacro with missing params should error, got: {}", r);
}

#[test]
fn test_defmacro_needs_body() {
    let r = eval("(defmacro mymac ())");
    assert!(r.starts_with("ERROR"), "defmacro with missing body should error, got: {}", r);
}

#[test]
fn test_quasiquote_needs_arg() {
    let r = eval("(quasiquote)");
    assert!(r.starts_with("ERROR"), "quasiquote without arg should error, got: {}", r);
}
