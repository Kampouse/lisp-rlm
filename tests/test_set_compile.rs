...
fn test_what_compiles_with_set_captured() {
    ...
    eval(
        r#"(define (inc! x) (set! x (+ x 1)) x)"#,
        &mut env,
        &mut state,
    );
    let inc = env.get("inc!").unwrap();
    match &inc {
        LispVal::Lambda { compiled, .. } => match compiled {
    ...
    eval(
        r#"(define (test-local-set) (let ((x 0)) (set! x 42) x))"#,
        &mut env,
        &mut state,
    );
    ...
    match &tls {
        LispVal::Lambda { compiled, .. } => match compiled {
            Some(cl) => eprintln!(
                "test-local-set (set! on let var) COMPILED: {} ops",
                cl.code.len()
            ),
    ...
    eval(
        r#"(define (make-adder base) (lambda (x) (set! base (+ base x)) base))"#,
        &mut env,
        &mut state,
    );
    ...
    match &adder {
        LispVal::Lambda { compiled, .. } => match compiled {
            Some(cl) => eprintln!(
                "adder (inner, set! on captured) COMPILED: {} ops",
                cl.code.len()
            ),
