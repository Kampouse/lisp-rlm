...
fn test_urgency_stale_over_one_hour() {
    ...
    assert!(
        (val - 0.7).abs() < 0.01,
        "stale intent should have urgency 0.7, got {}",
        val
    );
}
...
fn test_urgency_due_soon() {
    ...
    assert!(
        (val - 0.9).abs() < 0.01,
        "due soon should have urgency 0.9, got {}",
        val
    );
}
...
fn test_urgency_no_deadline_no_last_acted() {
    ...
    assert!(
        (val - 0.3).abs() < 0.01,
        "no urgency factors should be 0.3, got {}",
        val
    );
}
...
fn test_urgency_recent_acted_not_stale() {
    ...
    assert!(
        (val - 0.3).abs() < 0.01,
        "recently acted should be 0.3, got {}",
        val
    );
}
...
fn test_cost_efficiency_missing_cost() {
    ...
    assert!(
        (as_float(&e) - 0.9).abs() < 0.01,
        "missing cost → default 1 → 0.9, got {}",
        as_float(&e)
    );
}
...
fn test_load_patches_missing_dir() {
    ...
    assert!(matches!(
        result,
        LispVal::Nil | LispVal::Sym(_) | LispVal::Str(_)
    ));
}
...
fn test_mixed_types_one_shot_removed_perpetual_updated() {
    ...
    assert!(
        matches!(count, LispVal::Num(n) if n >= 2),
        "should have 2+ remaining, got {:?}",
        count
    );
}
...
fn test_multiple_one_shots_removed_one_per_tick() {
    ...
    assert!(
        matches!(after1, LispVal::Num(n) if n == 2),
        "should have 2 after first tick, got {:?}",
        after1
    );
    ...
    assert!(
        matches!(after2, LispVal::Num(n) if n == 1),
        "should have 1 after second tick, got {:?}",
        after2
    );
    ...
    assert!(
        matches!(after3, LispVal::Num(n) if n == 0),
        "should have 0 after third tick, got {:?}",
        after3
    );
}
...
fn test_scheduler_tracks_budget() {
    ...
    assert!(
        ...
        "budget used should be 0 or 5, got {:?}",
        used
    );
}
...
fn test_scheduler_skips_when_budget_zero() {
    ...
    assert_eq!(
        count,
        LispVal::Num(1),
        "intention should remain when budget exhausted"
    );
}
...
fn test_many_intentions_scheduler_picks_best() {
    ...
    assert!(
        matches!(remaining, LispVal::Num(n) if n == 2),
...
fn test_scheduler_processes_one_per_tick_with_many_intentions() {
    ...
    for i in 0..5 {
        eval(
            &format!(
                r#"(register-intention (dict "id" "os{}" "type" "one-shot" "cost" {}))"#,
                i,
                i * 10
            ),
            &mut env,
...
fn test_score_intention_missing_fields() {
    ...
    assert!(
        (val - 0.48).abs() < 0.01,
        "bare intent score should be ~0.48, got {}",
        val
    );
}
...
fn test_score_intention_overdue_cheap() {
    ...
    assert!(
        (val - 1.0).abs() < 0.01,
        "overdue+free should be 1.0, got {}",
        val
    );
}
