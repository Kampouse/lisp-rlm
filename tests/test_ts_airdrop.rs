...
fn airdrop_transfers_and_receipt_semantics() {
    ...
    assert!(
        ok.contains("dropped:2700000000000000000000000"),
        "success path: {ok}"
    );
    ...
    assert!(
        over.contains("a transfer receipt failed"),
        "resume fail-closed: {over}"
    );
    assert!(
        !over.contains("dropped:"),
        "resume aborted before recording"
    );
    ...
    assert!(
        s.contains("rita") && s.contains("raul"),
        "committed siblings persisted"
    );
    assert!(!s.contains("rosa"), "failed receipt reverted");
    assert!(
        !s.contains("ad:total"),
        "aborting callback reverted its writes"
    );
    ...
    assert!(
        again.contains("(bal now 1800000000000000000000000)"),
        "accumulating ledger: {again}"
    );
    let _ = NEAR;
}
...
fn mixed_batch_two_fn_calls_one_receipt() {
    ...
    let args =
        r#"{"minter":"air.ad.test.near","to":"bob.test.near","amt":"123000000000000000000000"}"#;
    ...
    assert!(
        out.contains("mints:2") || out.contains("receipt failed") || out.contains("TRAPPED"),
