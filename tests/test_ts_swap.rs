...
fn run(method: &str, input: &str, signer: Option<&str>, ts: Option<i64>) -> String {
    ...
    if let Some(t) = ts {
        cmd.env("NEAR_MOCK_BLOCK_TS", t.to_string());
    }
...
fn swap_happy_path_and_aborts() {
    ...
    assert!(run(
        "faMint",
        &format!(r#"{{"to":"alice.test.near","amount":"{mint}"}}"#),
        Some("alice.test.near"),
        Some(t0)
    )
    .contains(&format!("supply:{mint}")));
    assert!(run(
        "fbMint",
        &format!(r#"{{"to":"bob.test.near","amount":"{mint}"}}"#),
        Some("bob.test.near"),
        Some(t0)
    )
    .contains(&format!("supply:{mint}")));
    ...
    assert!(run(
        "swapClaimA",
        r#"{"id":"1","secret":"swap-secret-1"}"#,
        Some("bob.test.near"),
        Some(t0)
    )
    .contains("not in state B_CLAIMED"));
    assert!(run(
        "swapLockB",
        r#"{"id":"9"}"#,
        Some("bob.test.near"),
    ...
    .contains(&format!("claimedB:{amt_b}")));
    assert!(run(
        "swapClaimB",
        r#"{"id":"1","secret":"swap-secret-1"}"#,
        Some("alice.test.near"),
    ...
    .contains(&format!("claimedA:{amt_a}")));
    assert!(
        run("faBalanceOf", r#"{"who":"bob.test.near"}"#, None, Some(cla))
            .contains(&format!("📄 {amt_a}"))
    );
    assert!(run(
        "swapRefundB",
        r#"{"id":"1"}"#,
...
fn swap_abandonment_refunds() {
    ...
    run(
        "swapNew",
        &format!(r#"{{"amountA":"{amt_a}","amountB":"{amt_b}","timeoutSec":"300","secret":"s2"}}"#),
        Some("alice.test.near"),
        Some(t0),
    );
    assert!(run(
    ...
    .contains(&format!("refundedA:{amt_a}")));
    assert!(run(
        "faBalanceOf",
        r#"{"who":"alice.test.near"}"#,
        None,
    ...
    .contains(&format!("📄 {mint}")));

    // swap#2: both lock, alice never claims → bob at tlB, then alice at tlA
    run(
        "swapNew",
        &format!(r#"{{"amountA":"{amt_a}","amountB":"{amt_b}","timeoutSec":"300","secret":"s3"}}"#),
        Some("alice.test.near"),
        Some(t0),
    );
    run(
    ...
    .contains(&format!("refundedB:{amt_b}")));
    assert!(run(
        "swapRefundA",
        r#"{"id":"2"}"#,
        Some("alice.test.near"),
