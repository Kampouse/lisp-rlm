...
fn portfolio_parallel_fanout() {
    ...
    assert!(run(
        "tokb.pf.test.near",
        "ftMint",
        &format!(r#"{{"to":"alice.test.near","amount":"{}"}}"#, 250_000 * u),
        "alice.test.near"
    )
    .contains("supply:"));
    ...
    let out = run(
        "portfolio.pf.test.near",
        "portfolioTotal",
        r#"{"user":"alice.test.near"}"#,
        "alice.test.near",
    );
    assert!(
        out.contains(&format!("total:{}", 950_000 * u)),
    ...
    assert!(run(
        "toka.pf.test.near",
        "ftMint",
        &format!(r#"{{"to":"bob.test.near","amount":"{}"}}"#, 100_000 * u),
        "alice.test.near"
    )
    .contains("supply:"));
    assert!(run(
    ...
    .contains(&format!("total:{}", 100_000 * u)));
    ...
    .contains(&format!("total:{}", 950_000 * u)));
}
