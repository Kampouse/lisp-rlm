...
fn htlc_full_lifecycle() {
    ...
    assert!(run(
        &w,
        "ftMint",
        &format!(r#"{{"to":"owner.test.near","amount":"{mint}"}}"#),
    ...
    assert!(run(
        &w,
        "escrowClaim",
        r#"{"id":"1","secret":"s3cr3t-demo"}"#,
        Some("bob.test.near"),
        Some(ts1)
    )
    .contains("only the recipient may claim"));
    assert!(run(
        &w,
        "escrowRefund",
        r#"{"id":"1"}"#,
    ...
    assert!(run(
        ...
        "escrowNew",
        &format!(
            r#"{{"recipient":"bob.test.near","secret":"x2","amount":"{esc2}","timeoutSec":"1"}}"#
        ),
        Some("owner.test.near"),
        Some(ts3)
    )
    ...
    .contains(&format!("refunded:{esc2}")));
    assert!(run(
        &w,
        "ftBalanceOf",
        r#"{"who":"owner.test.near"}"#,
        None,
        Some(ts3)
    )
    .contains(&format!("📄 {}", mint - esc1)));
}
...
fn sha256_hash_is_hex_and_exact() {
    ...
    for (input, digest) in [
        ...
        (
            "s3cr3t-demo",
            "4821d08ebe7bae764da7fe2488b251b25b113280ac2eb8a02fcfdf2946f139ad",
        ),
    ] {
...
fn chained_jsonset_with_hash_no_longer_loses_keys() {
    ...
    let out = run(
        &w,
        "t",
        r#"{"recipient":"alice.test.near","secret":"s3cr3t-demo","amount":"500000000000000000000000","timeoutSec":"600"}"#,
        Some("owner.test.near"),
    ...
    assert!(
        out.contains(r#""sender":"owner.test.near""#),
        "record lost its keys: {out}"
    );
    assert!(
        out.contains(r#""hashlock":"4821d08e"#),
        "hashlock not hex-embedded: {out}"
    );
}
