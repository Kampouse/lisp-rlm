...
fn auction_full_flow() {
    ...
    let l = run(
        st,
        Call {
            acct: "auction.a.test.near",
            method: "list",
            args: r#"{"item":"nft:42","minBid":"10","endHeight":100}"#,
            attach: "0",
            height: "90",
            signer: "seller.test.near",
            view: false,
        },
    );
    ...
    assert!(
        b2.contains("transfer 10 yocto → alice.test.near"),
        "refund fired: {b2}"
    );
    let view = run(
        st,
    ...
    assert!(
        view.contains("bob.test.near") && view.contains("25"),
        "bob holds the top bid: {view}"
    );
