...
fn eval(src: &str, env: &mut Env, state: &mut EvalState) -> LispVal {
    ...
    lisp_rlm_wasm::program::run_program(&exprs, env, state)
...
fn test_find_best_three_items_compiled() {
    ...
    assert_eq!(
        best_id,
        LispVal::Str("high".into()),
        "find-best should pick highest score, got {:?}",
        best_id
    );
}
...
fn test_pick_best_three_items_compiled() {
    ...
    let best_id = eval(
        r#"(get-default (pick-best items (car items)) "id" "?")"#,
        &mut env,
        &mut state,
    );
    assert_eq!(
        best_id,
        LispVal::Str("high".into()),
        "pick-best should pick highest score, got {:?}",
        best_id
    );
}
...
fn test_recursive_gt_on_dict_get() {
    ...
    eval(
        r#"(define data (list (dict "id" "a" "score" 0.3) (dict "id" "b" "score" 0.9) (dict "id" "c" "score" 0.6)))"#,
        &mut env,
        &mut state,
    );

    let best_id = eval(
        r#"(get-default (my-find-best data) "id" "?")"#,
        &mut env,
        &mut state,
    );
    assert_eq!(
        best_id,
        LispVal::Str("b".into()),
        "my-find-best should pick b (score 0.9), got {:?}",
        best_id
    );
}
...
fn test_set_on_captured_var_compiled() {
    ...
    assert_eq!(
        r1,
        LispVal::Num(1),
        "first call should return 1, got {:?}",
        r1
    );
    ...
    assert_eq!(
        r2,
        LispVal::Num(2),
        "second call should return 2, got {:?}",
        r2
    );
}
