...
fn lending_liquidations_full_guard_battery() {
    ...
    let out = run(
        "deposit",
        r#"{"amt":"10000000000000000000000000"}"#,
        TS0,
        None,
    );
    assert!(
        out.contains(r#""dep":"10000000000000000000000000""#),
        "{out}"
    ); // 2026-09-01: jsonSet self-encodes (bug #22) — values now valid-JSON quoted
       ...
       // tracked separately.
    assert!(
        out.contains(r#""own":"owner.test.near""#),
        "first deposit must stamp owner: {out}"
    );

    // max borrow: debt lands EXACTLY on 5e24 (fee ceiled) → health 10000
    let out = run(
        "borrow",
        r#"{"amt":"4761904761904761904761904"}"#,
        TS0,
        None,
    );
    assert!(
        out.contains(r#""bor":"5000000000000000000000000""#),
        "{out}"
    );
    ...
    let out = run(
        "liquidate",
        r#"{"victim":"owner.test.near","amt":"1000000000000000000000000"}"#,
        TS0,
        Some("alice.test.near"),
    );
    assert!(
        out.contains("account healthy"),
        "at-line account must not be liquidatable: {out}"
    );
    ...
    let out = run(
        "liquidate",
        r#"{"victim":"owner.test.near","amt":"2000000000000000000000000"}"#,
        T100D,
        None,
    );
    ...
    let out = run(
        "liquidate",
        r#"{"victim":"owner.test.near","amt":"3000000000000000000000000"}"#,
        T100D,
        Some("alice.test.near"),
    );
    ...
    assert!(
        out.contains(r#""bor":"3136986301369863013698630""#),
        "{out}"
    );
    assert!(
        out.contains(r#""dep":"7900000000000000000000000""#),
        "{out}"
    );
    ...
    let out = run(
        "withdraw",
        r#"{"amt":"2000000000000000000000000"}"#,
        T100D,
        None,
    );
    ...
    let out = run(
        "withdraw",
        r#"{"amt":"1626027397260273972602740"}"#,
        T100D,
        None,
    );
    assert!(
        out.contains(r#""dep":"6273972602739726027397260""#),
        "{out}"
    );
    ...
    let out = run(
        "repay",
        r#"{"amt":"3136986301369863013698630"}"#,
        T100D,
        None,
    );
    ...
    let out = run(
        "withdraw",
        r#"{"amt":"6273972602739726027397260"}"#,
        T100D,
        None,
    );
