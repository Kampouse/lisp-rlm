...
fn ftlend_cdp_lifecycle() {
    ...
    .contains(&format!("debt:{cap}")));
    assert!(
        run("lendBorrow", r#"{"amount":"1"}"#, Some("owner.test.near"))
            .contains("would exceed borrow cap")
    );

    // 100% utilized: any withdraw undercollateralizes
    assert!(
        run("lendWithdraw", r#"{"amount":"1"}"#, Some("owner.test.near"))
            .contains("would undercollateralize")
    );

    // repay 1M → debt 3M; max withdraw = 2M (lands exactly on the line)
    assert!(run(
        "lendRepay",
        &format!(r#"{{"amount":"{repay}"}}"#),
        Some("owner.test.near")
    )
    .contains(&format!("debt:{debt2}")));
    assert!(run(
        "lendWithdraw",
        &format!(r#"{{"amount":"{wd_max}"}}"#),
        Some("owner.test.near")
    )
    .contains(&format!("locked:{}", dep - wd_max)));
    assert!(
        run("lendWithdraw", r#"{"amount":"1"}"#, Some("owner.test.near"))
            .contains("would undercollateralize")
    );
    ...
    assert!(
        run("lendHealth", "{}", Some("owner.test.near")).contains(&format!(
            "cap:{} debt:{}",
            3 * m * u,
            debt2
        ))
