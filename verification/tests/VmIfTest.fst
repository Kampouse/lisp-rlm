...
val vm_if_true : unit -> Lemma
  (match eval_steps 7 (fresh_vm [PushI64 1; JumpIfFalse 4; PushI64 42; Jump 5; PushI64 99; Return]) with
...
val vm_if_false : unit -> Lemma
  (match eval_steps 7 (fresh_vm [PushBool false; JumpIfFalse 4; PushI64 42; Jump 5; PushI64 99; Return]) with
   | LispIR.Semantics.Ok s' -> (match s'.stack with
     | Num r :: _ -> r = 99
     | _ -> false)
   | _ -> false)
...
val vm_if_nil : unit -> Lemma
  (match eval_steps 7 (fresh_vm [PushNil; JumpIfFalse 4; PushI64 42; Jump 5; PushI64 99; Return]) with
   | LispIR.Semantics.Ok s' -> (match s'.stack with
     | Num r :: _ -> r = 99
     | _ -> false)
   | _ -> false)
...
val vm_let : n:int -> Lemma
  (match eval_steps 7 (fresh_vm [PushI64 n; StoreSlot 0; LoadSlot 0; Return]) with
   | LispIR.Semantics.Ok s' -> (match s'.stack with
     | Num r :: _ -> r = n
     | _ -> false)
   | _ -> false)
...
val vm_not_true : unit -> Lemma
  (match eval_steps 7 (fresh_vm [PushBool true; JumpIfFalse 4; PushBool false; Jump 5; PushBool true; Return]) with
   | LispIR.Semantics.Ok s' -> (match s'.stack with
     | Bool r :: _ -> r = false
     | _ -> false)
   | _ -> false)
...
val vm_not_false : unit -> Lemma
  (match eval_steps 7 (fresh_vm [PushBool false; JumpIfFalse 4; PushBool false; Jump 5; PushBool true; Return]) with
   | LispIR.Semantics.Ok s' -> (match s'.stack with
     | Bool r :: _ -> r = true
     | _ -> false)
   | _ -> false)
