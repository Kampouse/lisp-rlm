...
(define (files-to-json files acc sep)
  ...
  (if (nil? files)
    (str-concat acc "]")
    (files-to-json (cdr files)
                   (str-concat acc sep "\"" (car files) "\"")
                   ",")))
...
(define (tasks-to-json tasks acc sep)
  (if (nil? tasks)
    (str-concat acc "]")
    (tasks-to-json (cdr tasks)
                   (str-concat acc sep (task-to-json (car tasks)))
                   ",")))
...
(define (call-ai prompt)
  (let ((body (str-concat
    ...
    prompt
    "\"}]}")))
    (let ((raw (http-post "https://api.z.ai/api/coding/paas/v4/chat/completions" body)))
...
(define (tick)
  (let ((phase (storage-get "ralph:phase")))
    (if (or (nil? phase) (= phase ""))
      (begin
        (storage-set "ralph:phase" "idle")
        (storage-set "ralph:agent:intent" "ralph-agent")
        "booted")
      (if (= phase "idle") (handle-idle)
        (if (= phase "implement") (handle-implement)
          (if (= phase "verify") (handle-verify)
            (if (= phase "done") (handle-done)
              (begin
                (storage-set "ralph:phase" "idle")
                "unknown-phase-reset"))))))))
