...
pub fn build_http_wit_metadata_embedded(
    ...
    resolve
        .push_group(pkg_deps_io)
        .map_err(|e| format!("push_group deps/io: {:?}", e))?;
    ...
    resolve
        .push_group(pkg_deps_clocks)
        .map_err(|e| format!("push_group deps/clocks: {:?}", e))?;
    ...
    resolve
        .push_group(pkg_deps_random)
        .map_err(|e| format!("push_group deps/random: {:?}", e))?;
    ...
    resolve
        .push_group(pkg_deps_filesystem)
        .map_err(|e| format!("push_group deps/filesystem: {:?}", e))?;
    ...
    resolve
        .push_group(pkg_deps_sockets)
        .map_err(|e| format!("push_group deps/sockets: {:?}", e))?;
    ...
    resolve
        .push_group(pkg_deps_cli)
        .map_err(|e| format!("push_group deps/cli: {:?}", e))?;
    ...
    resolve
        .push_group(pkg_deps_http)
        .map_err(|e| format!("push_group deps/http: {:?}", e))?;

    // outlayer:api/host removed — upstream uses split interfaces, not loaded for P1

    let pkg_main = build_group(&[("world.wit", WORLD_WIT)])?;
    let pkg_id = resolve
        .push_group(pkg_main)
        .map_err(|e| format!("push_group main: {:?}", e))?;
