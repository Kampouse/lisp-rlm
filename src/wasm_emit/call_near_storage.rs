...
impl WasmEmitter {
    ...
    pub(crate) fn emit_storage_cache_flush(v: &mut Vec<Instruction<'static>>) {
        ...
        v.push(Instruction::I64Store(MemArg {
            offset: 0,
            align: 3,
            memory_index: 0,
        }));
    ...
    pub(crate) fn emit_storage_cache_lookup(
        ...
        v.push(Instruction::I64Add);
        v.push(Instruction::I32WrapI64);
        v.push(Instruction::I64Load8U(MemArg {
            offset: 24,
            align: 0,
            memory_index: 0,
        }));
    ...
    pub(crate) fn emit_storage_cache_insert(
        ...
        v.push(Instruction::I32WrapI64);
        v.push(Instruction::I64Load8U(ma0.clone()));
        v.push(Instruction::I64Store8(MemArg {
            offset: 24,
            align: 0,
            memory_index: 0,
        }));
    ...
    pub(crate) fn call_near_storage(
        ...
        match op {
            ...
            "near/storage_get" | "near/storage_read" => {
                ...
                v.push(Instruction::LocalSet(res_l));
                v.extend(
                    self.emit_storage_cache_insert(k, res_l, cnt_l, slot_l, j_l, klen_l, kptr_l),
                );
                v.push(Instruction::End);
                v.push(Instruction::LocalGet(res_l));
                Ok(v)
            ...
            "near/kstore" => {
                if a.len() != 3 {
                    return Err("near/kstore requires 3 args: prefix account value".into());
                }
                self.need_host(17);
                self.need_host(0);
                self.need_host(1);
                ...
                v.extend(self.emit_untag());
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64ShrU); // prefix_len (i64)
                ...
                v.extend(self.emit_untag());
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64ShrU); // account_len (i64)
            ...
            "near/kload" => {
                if a.len() != 2 {
                    return Err("near/kload requires 2 args: prefix account".into());
                }
                let prefix_expr = self.expr(&a[0])?;
                let acct_expr = self.expr(&a[1])?;
