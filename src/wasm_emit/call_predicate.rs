...
impl WasmEmitter {
    pub(crate) fn call_predicate(
        ...
        match op {
            "number?" => {
                if a.len() != 1 {
                    return Err("number?: expected 1 arg".into());
                }
            ...
            "zero?" => {
                if a.len() != 1 {
                    return Err("zero?: expected 1 arg".into());
                }
                let mut v = self.expr(&a[0])?;
            ...
            "nil?" => {
                if a.len() != 1 {
                    return Err("nil?: expected 1 arg".into());
                }
                let mut v = self.expr(&a[0])?;
            ...
            "list?" => {
                if a.len() != 1 {
                    return Err("list?: expected 1 arg".into());
                }
                let mut v = self.expr(&a[0])?;
                v.push(Instruction::I64Const(7));
                v.push(Instruction::I64And);
            ...
            "bool?" => {
                if a.len() != 1 {
                    return Err("bool?: expected 1 arg".into());
                }
                let mut v = self.expr(&a[0])?;
                v.push(Instruction::I64Const(7));
                v.push(Instruction::I64And);
            ...
            "string?" => {
                if a.len() != 1 {
                    return Err("string?: expected 1 arg".into());
                }
                let mut v = self.expr(&a[0])?;
                v.push(Instruction::I64Const(7));
                v.push(Instruction::I64And);
