    ...
    pub(crate) fn call_defi(
        ...
        match op {
            ...
            "sqrt" => {
                ...
                let mut v = Vec::new();
                v.extend(x);
                v.push(Instruction::LocalSet(x_i));
                // if x == 0: return 0
                ...
                v.push(Instruction::Loop(BlockType::Empty));
                v.push(Instruction::LocalGet(r_i));
                v.push(Instruction::LocalSet(prev_i));
                // r = (r + x/r) / 2
                v.push(Instruction::LocalGet(r_i));
                v.push(Instruction::LocalGet(x_i));
                v.push(Instruction::LocalGet(r_i));
            ...
            "price_to_tick" => {
                ...
                let mut v = Vec::new();
                v.extend(p);
                v.push(Instruction::LocalSet(p_i));
                ...
                v.push(Instruction::I64Const(64));
                v.push(Instruction::I64ShrU); // to integer
                v.push(Instruction::I64Const(10000));
                v.push(Instruction::I64Mul);
                Ok(v)
            }
            "liq_amount0" => {
                let spa = self.expr(&a[0])?;
                let spb = self.expr(&a[1])?;
                let liq = self.expr(&a[2])?;
                let spa_i = self.local_idx("__la0_a");
                let spb_i = self.local_idx("__la0_b");
                let liq_i = self.local_idx("__la0_l");
                let mut v = Vec::new();
                v.extend(spa);
                v.push(Instruction::LocalSet(spa_i));
                v.extend(spb);
                v.push(Instruction::LocalSet(spb_i));
                v.extend(liq);
                v.push(Instruction::LocalSet(liq_i));
                // numerator = liq * (spb - spa) — Q64.64 mul
                v.push(Instruction::LocalGet(liq_i));
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64ShrU);
                v.push(Instruction::LocalGet(spb_i));
                v.push(Instruction::LocalGet(spa_i));
                v.push(Instruction::I64Sub);
                v.push(Instruction::I64Mul);
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64Shl);
            ...
            "liq_amount1" => {
                ...
                let spb = self.expr(&a[1])?;
                let liq = self.expr(&a[2])?;
                let spa_i = self.local_idx("__la1_a");
                let spb_i = self.local_idx("__la1_b");
                let liq_i = self.local_idx("__la1_l");
                let mut v = Vec::new();
                v.extend(spa);
                v.push(Instruction::LocalSet(spa_i));
                v.extend(spb);
            ...
            "liq_amount0_64" => {
                ...
                v.extend(spa_a);
                v.push(Instruction::LocalSet(spa_lo)); // spa addr
                v.extend(spb_a);
                v.push(Instruction::LocalSet(spb_lo)); // spb addr
                v.extend(liq_a);
                v.push(Instruction::LocalSet(liq_hi)); // liq addr
                                                       // Load spa Q64.64
                v.push(Instruction::LocalGet(spa_lo));
                v.push(Instruction::I32WrapI64);
                ...
                v.push(Instruction::LocalGet(diff_lo));
                v.push(Instruction::LocalSet(spa_lo)); // proper spa_lo
                                                       // Load spb Q64.64
                v.push(Instruction::LocalGet(spb_lo));
                v.push(Instruction::I32WrapI64);
                v.push(Instruction::I64Load(wasm_encoder::MemArg {
                    offset: 0,
                    align: 3,
                ...
                v.push(Instruction::LocalGet(spb_lo));
                v.push(Instruction::I32WrapI64); // need spb addr for hi
                ...
                v.extend(spa_e);
                v.push(Instruction::LocalSet(addr_spa));
                let spb_e = self.expr(&a[2])?;
                v.extend(spb_e);
                v.push(Instruction::LocalSet(addr_spb));
                let liq_e = self.expr(&a[3])?;
                v.extend(liq_e);
                v.push(Instruction::LocalSet(addr_liq));
                // Load spa
                v.push(Instruction::LocalGet(addr_spa));
                v.push(Instruction::I32WrapI64);
                v.push(Instruction::I64Load(wasm_encoder::MemArg {
                    offset: 8,
                    align: 3,
                ...
                v.push(Instruction::LocalSet(spa_hi));
                // Load spb
                v.push(Instruction::LocalGet(addr_spb));
                v.push(Instruction::I32WrapI64);
                v.push(Instruction::I64Load(wasm_encoder::MemArg {
                    offset: 8,
                    align: 3,
                ...
                // Load liq
                v.push(Instruction::LocalGet(addr_liq));
                v.push(Instruction::I32WrapI64);
                v.push(Instruction::I64Load(wasm_encoder::MemArg {
                    offset: 8,
                    align: 3,
                ...
                v.push(Instruction::LocalGet(spb_hi));
                v.push(Instruction::LocalGet(spa_hi));
                v.push(Instruction::I64Sub);
                v.push(Instruction::LocalSet(diff_hi));
                // numerator = liq_hi * diff_hi
                v.push(Instruction::LocalGet(liq_hi));
                v.push(Instruction::LocalGet(diff_hi));
                v.push(Instruction::I64Mul);
                ...
                v.push(Instruction::LocalGet(den_hi));
                v.push(Instruction::I64DivU);
                v.push(Instruction::LocalSet(num_hi));
            ...
            "liq_amount1_64" => {
                ...
                v.push(Instruction::LocalGet(spb_h));
                v.push(Instruction::LocalGet(spa_h));
                v.push(Instruction::I64Sub);
                v.push(Instruction::I64Mul);
                v.push(Instruction::LocalSet(liq_h));
                v.push(Instruction::LocalGet(dst_i));
            ...
            "price64_to_tick" => {
                ...
                // q32 = (ph << 32) | (pl >> 32)
                v.push(Instruction::LocalGet(ph));
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64Shl);
                v.push(Instruction::LocalGet(pl));
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64ShrU);
                ...
                // tick = diff * 10001 >> 32
                v.push(Instruction::LocalGet(diff));
                v.push(Instruction::I64Const(10001));
                v.push(Instruction::I64Mul);
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64ShrU);
                ...
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64ShrU);
                v.push(Instruction::I64Const(5002));
                v.push(Instruction::I64Mul);
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64ShrU);
                v.push(Instruction::I64Sub);
                v.push(Instruction::LocalSet(tick));
                v.push(Instruction::LocalGet(tick));
                Ok(v)
            }
