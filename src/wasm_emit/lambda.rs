...
impl WasmEmitter {
    pub(crate) fn extract_lambda(form: &LispVal) -> Result<(String, LispVal), String> {
        match form {
            LispVal::List(items) if items.len() >= 3 => {
                ...
                Err(format!(
                    "hof: expected (lambda (param) body), got {:?}",
                    form
                ))
            }
    ...
    pub(crate) fn extract_lambda_2param(form: &LispVal) -> Result<(Vec<String>, LispVal), String> {
        match form {
            LispVal::List(items) if items.len() >= 3 => {
                ...
                Err(format!(
                    "hof/reduce: expected (lambda (acc x) body), got {:?}",
                    form
                ))
            }
    ...
    pub(crate) fn parse_u128(s: &str) -> Result<(i64, i64), String> {
        ...
        for ch in s.chars() {
            ...
            if ch < '0' || ch > '9' {
                return Err(format!("invalid digit in u128 literal: '{}'", ch));
            }
    ...
    pub(crate) fn has_tc(&self, e: &LispVal) -> bool {
        ...
        let a = &items[1..];
        if Some(op.as_str()) == self.current_func.as_deref() && a.len() == self.current_param_count
        {
            return true;
        }
        if op == "if" {
            return self.has_tc(&a[1]) || (a.len() > 2 && self.has_tc(&a[2]));
        }
        if op == "begin" && !a.is_empty() {
            return self.has_tc(items.last().unwrap());
        }
        if op == "let" && a.len() > 1 {
            return a[1..].iter().any(|x| self.has_tc(x));
        }
        false
    }
    ...
    pub(crate) fn is_self(&self, e: &LispVal) -> bool {
        ...
        Some(op.as_str()) == self.current_func.as_deref()
            && items.len() - 1 == self.current_param_count
    }
    ...
    pub(crate) fn collect_free(
        ...
        match e {
            LispVal::Sym(s) => {
                if !bound.contains(s)
                    && self.locals.contains_key(s)
                    && !self.funcs.iter().any(|f| f.name == *s)
                {
                    free.insert(s.clone());
                }
            }
            LispVal::List(items) if !items.is_empty() => {
                ...
                for item in items {
                    self.collect_free(item, bound, free);
                }
            }
            _ => {}
        }
    ...
    pub(crate) fn emit_lambda(
        ...
        if captured.is_empty() {
            ...
            v.push(Instruction::I64Const(
                ((lambda_id as i64) << TAG_BITS) | TAG_FNREF,
            ));
        } else {
            ...
            for (i, cap) in captured.iter().enumerate() {
                let &local_idx = self
                    .locals
                    .get(cap)
                    .ok_or_else(|| format!("lambda capture: undef local {}", cap))?;
            ...
            v.push(Instruction::I64Const(
                ((ptr as i64) << TAG_BITS) | TAG_CLOSURE,
            ));
        }
        Ok(v)
    }
    ...
    pub(crate) fn scan_host(&mut self, e: &LispVal) {
        ...
        match op.as_str() {
            ...
            "near/json_get_int" | "near/json_get_str" | "near/json_get_u128" | "json-get"
            | "json-get-str" | "json-get-float" | "json/get" => {
                if !self.wasi_mode {
                    self.need_host(7);
                    self.need_host(0);
                    self.need_host(1);
            ...
            "near/json_return_int" | "near/json_return_str" | "json-return" => {
                if !self.wasi_mode {
                    self.need_host(25);
                }
            }
            ...
            "outlayer/http-post" | "http-post" => {
                // HTTP POST → combined P2 path (works with inlayer)
                self.need_outlayer = true;
    ...
    pub(crate) fn resolve_lambda_1(
        ...
        match arg {
            ...
            LispVal::List(items)
                if items.len() >= 3
                    && matches!(&items[0], LispVal::Sym(s) if s == "fn" || s == "lambda") =>
            {
                let pname = match &items[1] {
                    LispVal::Sym(s) => s.clone(),
                    LispVal::List(ps) if !ps.is_empty() => match &ps[0] {
                        LispVal::Sym(s) => s.clone(),
                        _ => "x".into(),
                    },
                    _ => "x".into(),
            ...
            _ => Err(format!(
                "{}: first arg must be (fn [x] body) or named function",
                ctx
            )),
        }
    }
    ...
    pub(crate) fn resolve_lambda_2(
        &self,
        arg: &LispVal,
        ctx: &str,
    ) -> Result<(String, String, LispVal), String> {
        match arg {
            LispVal::List(items)
                if items.len() >= 3
                    && matches!(&items[0], LispVal::Sym(s) if s == "fn" || s == "lambda") =>
            ...
            _ => Err(format!(
                "{}: first arg must be (fn [a b] body) or named function",
                ctx
            )),
        }
    }
