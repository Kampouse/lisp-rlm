    ...
    pub(crate) fn call_outlayer(
        ...
        match op {
            ...
            "http-post" => {
                ...
                // Always uses outlayer host function path (has auth injection)
                if a.len() < 2 {
                ...
                // Always use outlayer host function path (has auth injection via env vars)
            ...
            "outlayer/view" => {
                ...
                if !self.wasi_mode {
                    return Err("outlayer/view is only available on OutLayer".into());
                }
                self.need_outlayer = true;
            ...
            "outlayer/sleep-ms" => {
                ...
                return self.call_outlayer("sleep-ms", a);
            }
            "outlayer/send-telegram" => {
                // (outlayer/send-telegram "chat-id" "text") -> string or nil
            ...
            "outlayer/transfer" => {
                ...
                if a.len() != 4 {
                    return Err("outlayer/transfer requires 4 args: signer_id signer_key receiver amount_yocto".into());
                }
                let ma4 = wasm_encoder::MemArg {
                    offset: 0,
                    align: 2,
            ...
            "near/call-signed" => {
                ...
                if a.len() < 7 {
                    return Err("near/call-signed requires 7-8 args: signer-id signer-key receiver method args deposit gas [wait-until]".into());
                }
                if !self.wasi_mode {
                    return Err("near/call-signed is only available on OutLayer".into());
                }
                self.need_outlayer = true;
                let ma4 = wasm_encoder::MemArg {
                    offset: 0,
            ...
            "near/transfer-signed" => {
                ...
                // Returns tx_hash on success, nil on error.
                if a.len() < 4 {
                    return Err("near/transfer-signed requires 4-5 args: signer-id signer-key receiver amount-yocto [wait-until]".into());
                }
                if !self.wasi_mode {
                    return Err("near/transfer-signed is only available on OutLayer".into());
                }
                self.need_outlayer = true;
                let ma4 = wasm_encoder::MemArg {
                    offset: 0,
            ...
            "sleep-ms" => {
                ...
                if a.is_empty() {
                    return Err("sleep-ms requires a duration in milliseconds (number)".into());
                }
                if !self.wasi_mode {
                    return Err("sleep-ms is only available on OutLayer".into());
                }
                self.need_outlayer = true;
                let ms_expr = self.expr(&a[0])?;
                let ma4 = wasm_encoder::MemArg {
                    offset: 0,
                    align: 2,
                    memory_index: 0,
                ...
                let ret_area: i32 = crate::wasi_http::OL_RET_AREA_BASE + 512;
                let mut v = Vec::new();
                // Push ms as i32 (untag number → i64 → i32.trunc)
                v.extend(ms_expr);
                v.extend(self.emit_untag()); // i64 raw value
                v.push(Instruction::I32WrapI64); // i32
                                                 // Push ret_area
                v.push(Instruction::I32Const(ret_area));
            ...
            "http-post-dynamic" => {
                ...
                if a.len() < 2 {
                    return Err(
                        "http-post-dynamic requires (url body) or (url body content-type)".into(),
                    );
                }
                if !self.wasi_mode {
                    return Err(
                        "http-post-dynamic is only available on OutLayer (WASI) target".into(),
                    );
                }
                self.need_outlayer = true;
            ...
            "send-telegram" => {
                // (send-telegram "chat-id" "text") -> string or nil
                // WIT: send-telegram(chat_id: string, text: string) -> result<string, string>
                // Canonical ABI: (chat_id_ptr, chat_id_len, text_ptr, text_len, ret_area) -> ()
                // ret_area layout: [i32 disc][4 pad][ptr result_str][len result_str]
                // Sentinel 142 → outlayer_imports index 18 (send-telegram)
                if a.len() < 2 {
                    return Err("send-telegram requires (chat_id text)".into());
                }
                if !self.wasi_mode {
                    return Err("send-telegram is only available on OutLayer".into());
                }
                self.need_outlayer = true;
                let chat_expr = self.expr(&a[0])?;
                let text_expr = self.expr(&a[1])?;
                let ma4 = wasm_encoder::MemArg {
                    offset: 0,
                    align: 2,
                    memory_index: 0,
                };
                let ret_area: i32 = crate::wasi_http::OL_RET_AREA_BASE + 1024;
                let mut v = Vec::new();
                // Push chat_id string (ptr, len)
                v.extend(chat_expr.clone());
                v.extend(self.emit_untag());
                v.push(Instruction::I64Const(0xFFFFFFFF));
                v.push(Instruction::I64And);
                v.push(Instruction::I32WrapI64); // chat_ptr
                v.extend(chat_expr);
                v.extend(self.emit_untag());
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64ShrU);
                v.push(Instruction::I32WrapI64); // chat_len
                                                 // Push text string (ptr, len)
                v.extend(text_expr.clone());
                v.extend(self.emit_untag());
                v.push(Instruction::I64Const(0xFFFFFFFF));
                v.push(Instruction::I64And);
                v.push(Instruction::I32WrapI64); // text_ptr
                v.extend(text_expr);
                v.extend(self.emit_untag());
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64ShrU);
                v.push(Instruction::I32WrapI64); // text_len
                                                 // Push ret_area
                v.push(Instruction::I32Const(ret_area));
                // Call send-telegram sentinel
                v.push(Instruction::Call(142));
                // Read discriminant from ret_area (0 = Ok, 1 = Err)
                v.push(Instruction::I32Const(ret_area));
                v.push(Instruction::I32Load(ma4));
                v.push(Instruction::If(BlockType::Result(ValType::I64)));
                v.push(Instruction::I64Const(0)); // TAG_NIL on error
                v.push(Instruction::Else);
                // Ok path: read result string (ptr @ ret_area+8, len @ ret_area+12)
                v.push(Instruction::I32Const(ret_area + 8));
            ...
            "web-search" => {
                // (web-search "query") -> string or nil
                // WIT: web-search(query: string) -> result<string, string>
                // Canonical ABI: (query_ptr, query_len, ret_area) -> ()
                // ret_area layout: [i32 disc][4 pad][ptr result_str][len result_str]
                // Sentinel 144 → outlayer_imports index 20 (web-search)
                if a.is_empty() {
                    return Err("web-search requires (query)".into());
                }
                if !self.wasi_mode {
                    return Err("web-search is only available on OutLayer".into());
                }
                self.need_outlayer = true;
                let query_expr = self.expr(&a[0])?;
                let ma4 = wasm_encoder::MemArg {
                    offset: 0,
                    align: 2,
                    memory_index: 0,
                ...
                let mut v = Vec::new();
                // Push query string (ptr, len)
                v.extend(query_expr.clone());
                v.extend(self.emit_untag());
                v.push(Instruction::I64Const(0xFFFFFFFF));
                v.push(Instruction::I64And);
                v.push(Instruction::I32WrapI64); // query_ptr
                v.extend(query_expr);
                v.extend(self.emit_untag());
                v.push(Instruction::I64Const(32));
                v.push(Instruction::I64ShrU);
                v.push(Instruction::I32WrapI64); // query_len
                                                 // Push ret_area
                v.push(Instruction::I32Const(ret_area));
                // Call web-search sentinel
                v.push(Instruction::Call(144));
                // Read discriminant from ret_area (0 = Ok, 1 = Err)
                v.push(Instruction::I32Const(ret_area));
                v.push(Instruction::I32Load(ma4));
                v.push(Instruction::If(BlockType::Result(ValType::I64)));
            ...
            "ai-chat" => {
                ...
                if a.len() != 1 {
                    return Err("ai-chat requires 1 argument: (ai-chat \"prompt\")".into());
                }
                if !self.wasi_mode {
                    return Err("ai-chat is only available on OutLayer".into());
                }
                self.need_outlayer = true;
            ...
            "rpc-call" => {
                ...
                if a.len() < 2 {
                    return Err("rpc-call requires (method params_json)".into());
                }
                if !self.wasi_mode {
                    return Err("rpc-call is only available on OutLayer".into());
                }
                self.need_outlayer = true;
