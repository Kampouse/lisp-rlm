...
impl WasmEmitter {
    pub(crate) fn call_json(
        ...
        match op {
            ...
            "json-get-str" => {
                if a.is_empty() {
                    return Err("json-get-str requires a string key argument".into());
                }
                match &a[0] {
                    LispVal::Str(key) => {
                        let mut v = if a.len() > 1 {
                            ...
                            setup.push(Instruction::LocalSet(jgs_len));
                            setup.push(Instruction::LocalGet(jgs_tmp));
                            setup.push(Instruction::I32WrapI64);
                            setup.push(Instruction::LocalSet(jgs_ptr));
                            ...
                            setup.push(Instruction::MemoryCopy {
                                src_mem: 0,
                                dst_mem: 0,
                            });
