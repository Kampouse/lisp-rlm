...
impl WasmEmitter {
    ...
    fn array_to_str_code(&mut self) -> Vec<Instruction<'static>> {
        let ma8 = |off: u64| wasm_encoder::MemArg {
            offset: off,
            align: 0,
            memory_index: 0,
        };
        ...
        v.push(lg(base));
        v.push(Instruction::I32WrapI64);
        v.push(Instruction::I64Load(ma64.clone()));
        v.push(ls(cnt));
        ...
        v.push(Instruction::If(BlockType::Empty));
        // "false"
        store8_at(&mut v, cur, 0, 102);
        store8_at(&mut v, cur, 1, 97);
        store8_at(&mut v, cur, 2, 108);
        store8_at(&mut v, cur, 3, 115);
        ...
        v.push(Instruction::Else);
        // "true"
        store8_at(&mut v, cur, 0, 116);
        store8_at(&mut v, cur, 1, 114);
        store8_at(&mut v, cur, 2, 117);
        store8_at(&mut v, cur, 3, 101);
        v.push(lg(cur));
        v.push(n(4));
        v.push(Instruction::I64Add);
        v.push(ls(cur));
        ...
        v.push(Instruction::I64Eq);
        v.push(Instruction::If(BlockType::Empty));
        store8_at(&mut v, cur, 0, 110);
        store8_at(&mut v, cur, 1, 105);
        ...
        v.push(Instruction::I64Eq);
        v.push(Instruction::If(BlockType::Empty));
        store8_at(&mut v, cur, 0, 60);
        store8_at(&mut v, cur, 1, 118);
        store8_at(&mut v, cur, 2, 101);
        store8_at(&mut v, cur, 3, 99);
    ...
    pub(crate) fn int_to_str_clean_from(
        ...
        v.push(Instruction::I64Eq);
        v.push(Instruction::If(BlockType::Result(
