...
pub fn compile_outlayer_p2_from_exprs(exprs: &[crate::types::LispVal]) -> Result<Vec<u8>, String> {
    ...
    eprintln!(
        "[DEBUG] need_outlayer={}, need_wasi_http={}, funcs={}",
        em.need_outlayer,
        em.need_wasi_http,
        em.funcs.len()
    );
...
pub fn compile_outlayer_p2(source: &str) -> Result<Vec<u8>, String> {
    ...
    eprintln!(
        "[DEBUG] compile_outlayer_p2: need_outlayer={}, need_wasi_http={}, funcs={}",
        em.need_outlayer,
        em.need_wasi_http,
        em.funcs.len()
    );
...
fn build_p2_with_wasi_http(em: &WasmEmitter) -> Result<Vec<u8>, String> {
    ...
    // ═══ Data Section (string literals + URL data segments) ═══
    {
        ...
        // core data. Set it to 1000000 (above lisp heap range 200000-999980).
        {
            ...
            data.active(
                0,
                &ConstExpr::i32_const(900000),
                adapter_bump.iter().copied(),
            );
            has_data = true;
        }
...
fn finish_outlayer_inner(em: &mut WasmEmitter, skip_outlayer: bool) -> Result<Vec<u8>, String> {
    ...
    let http_get_used = http_url_count > 0
        && em.funcs.iter().any(|f| {
            f.instrs.iter().any(
    ...
    let http_get_import_idx = if http_get_used {
        ...
        imports.import(
            hf.module,
            hf.name,
            EntityType::Function(ol_type_map_full[3]),
        );
        total_imports += 1;
        idx
    } else {
    ...
    // Reads stdin → tagged string → calls run(input) → writes result to stdout
    {
        ...
        if std::env::var("OUTLAYER_DEBUG").is_ok() {
            eprintln!(
                "funcs: {:?}",
                em.funcs.iter().map(|f| f.name.as_str()).collect::<Vec<_>>()
            );
...
pub(crate) fn emit_fd_write_shim(
    ...
    fw.instruction(&Instruction::LocalGet(9));
    fw.instruction(&Instruction::I32Load(MemArg {
        offset: 4,
        align: 2,
        memory_index: 0,
    }));
...
fn build_combined_p2_core(em: &mut WasmEmitter) -> Result<(Vec<u8>, bool), String> {
    ...
    let bridge_native_post =
        em.http_post_call_count > 0 && em.http_post_urls.len() as u32 == em.http_post_call_count;
    if bridge_native_post {
        used_ol_indices.remove(&21); // http-post-dynamic import no longer needed
    }
    eprintln!(
        "[DEBUG] used_ol_indices: {:?} (bridge_native_post={})",
        used_ol_indices, bridge_native_post
    );
    ...
    let ol_type_map_full: Vec<u32> = vec![
        ...
        ol_type_3,
        ol_type_7, // 3: http-get, 4: http-post
        ol_type_5,
        ol_type_3, // 5: set, 6: get
        ol_type_2ret,
        ol_type_2ret, // 7: has, 8: delete (bool return)
        ol_type_s64,
        ol_type_s64, // 9: increment, 10: decrement
        ol_type_5,
        ol_type_7, // 11: set-if-absent, 12: set-if-equals
        ol_type_3,
        ol_type_1, // 13: list-keys, 14: clear-all
        ol_type_7,
        ol_type_6, // 15: set-worker, 16: get-worker
        ol_type_5, // 17: raw
    ...
    emit_fd_write_shim(
        &mut codes,
        crate::wasi_http::FN_GET_STDOUT,
        crate::wasi_http::FN_OUTPUT_STREAM_WRITE,
    );
