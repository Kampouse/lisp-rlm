...
impl WasmEmitter {
    ...
    fn scan_host(&mut self, e: &LispVal) {
        ...
        match op.as_str() {
            ...
            "near/json_get_int" | "near/json_get_str" => { self.need_host(7); self.need_host(0); self.need_host(1); }
...
impl<'a> TypeChecker<'a> {
    ...
    fn check_call(&mut self, op: &str, args: &[LispVal]) -> Result<Ty, String> {
        ...
        match op {
            ...
            "near/predecessor_account_id" | "near/input" | "near/json_get_int" |
            "near/block_index" |
    ...
    fn suggest_function(&self, name: &str) -> Option<String> {
        ...
        let builtins = ["+", "-", "*", "/", "mod", "abs", "=", "!=", "<", ">", "<=", ">=",
            "and", "or", "not", "if", "let", "begin", "while", "for", "set!", "quote",
            "near/log", "near/return", "near/store", "near/load", "near/storage_set", "near/storage_get", "near/storage_has", "near/storage_remove", "near/json_get_int", "near/json_get_str", "near/json_return_int", "near/json_return_str", "hof/map", "hof/filter", "hof/reduce"];
        for b in &builtins {
