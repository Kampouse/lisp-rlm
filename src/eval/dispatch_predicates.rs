...
pub fn handle(name: &str, args: &[LispVal]) -> Result<Option<LispVal>, String> {
    match name {
        ...
        "number?" => Ok(Some(LispVal::Bool(matches!(&args[0], LispVal::Num(_) | LispVal::Float(_))))),
        ...
        "macro?" => Ok(Some(LispVal::Bool(matches!(&args[0], LispVal::Macro { .. })))),
        "type?" => Ok(Some(LispVal::Str(match &args[0] {
            LispVal::Nil => "nil",
        ...
        "to-float" => match &args[0] {
            ...
            LispVal::Str(s) => s.parse::<f64>().map(LispVal::Float).map(Some).map_err(|_| format!("to-float: cannot parse '{}'", s)),
        ...
        "to-int" => match &args[0] {
            ...
            LispVal::Str(s) => s.parse::<i64>().map(LispVal::Num).map(Some).map_err(|_| format!("to-int: cannot parse '{}'", s)),
            other => Err(format!("to-int: expected number, got {}", other)),
        },
        "to-num" => match &args[0] {
            LispVal::Num(n) => Ok(Some(LispVal::Num(*n))),
            LispVal::Float(f) => Ok(Some(LispVal::Num(*f as i64))),
            LispVal::Str(s) => s.parse::<i64>().map(LispVal::Num).map(Some).map_err(|_| format!("to-num: cannot parse '{}'", s)),
