...
fn main() {
    ...
    let is_outlayer = args.iter().any(|a| a == "--target" && args.iter().position(|x| x == "--target").map(|i| args.get(i+1).map(|v| v == "outlayer" || v == "outlayer-p2").unwrap_or(false)).unwrap_or(false))
