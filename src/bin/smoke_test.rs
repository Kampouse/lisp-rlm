...
fn eval(s: &str, env: &mut Env, state: &mut EvalState) -> String {
    ...
    for e in &exprs { match lisp_eval(e, env, state) { Ok(v) => r = format!("{:?}", v), Err(e) => r = format!("ERR: {}", e) } }
    r
}
...
fn main() {
    ...
    // 2-arg inline lambda
    println!("2-arg call: {}", eval("((lambda (a b) (+ a b)) 3 4)", &mut env, &mut state));
...
"#;
    ...
    println!("gcd2(1071,462): {}", eval("(gcd2 1071 462)", &mut env, &mut state));
...
"#;
    ...
    println!("foldl sum 1..5: {}", eval("(foldl-y + 0 (list 1 2 3 4 5))", &mut env, &mut state));
    println!("foldl sum 1..100: {}", eval("(foldl-y + 0 (range 1 101))", &mut env, &mut state));
    println!("foldl * 1..6: {}", eval("(foldl-y * 1 (range 1 7))", &mut env, &mut state));
}
