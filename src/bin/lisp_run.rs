...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "--coverage" => {
            if args.len() < 3 {
                eprintln!("--coverage requires a file");
                std::process::exit(1);
            }
        ...
        "--verify" => {
            if args.len() < 3 {
                eprintln!("--verify requires a file");
                std::process::exit(1);
            }
            verify_file(&args[2]);
        }
...
fn display_val(v: &LispVal) -> String {
    match v {
        ...
        LispVal::List(items) => {
            let i: Vec<String> = items.iter().map(display_val).collect();
            format!("({})", i.join(" "))
        }
        LispVal::Map(m) => {
            let p: Vec<String> = m
                .iter()
                .map(|(k, v)| format!("{} {}", k, display_val(v)))
                .collect();
            format!("(dict {})", p.join(" "))
        }
        _ => format!("{:?}", v),
...
fn values_match(actual: &str, expected: &str) -> bool {
        ...
        && expected.ends_with(')')
    {
        ...
        if a.len() == e.len() {
            return a.iter().zip(e.iter()).all(|(a, e)| values_match(a, e));
        }
    }
    false
}
...
fn eval_and_print(code: &str, verbose: bool) {
    ...
    for (i, expr) in exprs.iter().enumerate() {
        ...
        match lisp_rlm_wasm::program::run_program(&[expr.clone()], &mut env, &mut state) {
            Ok(v) => {
                if verbose {
                    eprintln!("  => {:?}", truncate_display(&v, 60));
                }
                result = v;
            }
            Err(e) => {
                eprintln!("ERROR at form {}: {}", i, e);
                std::process::exit(1);
            }
        }
    }
    ...
    if verbose {
        eprintln!("Evaluated in {:?}", start.elapsed());
    }
}
...
fn check_file(path: &str) {
    ...
    let exprs = match parse_all(&strip_line_comments(&code)) {
        Ok(e) => e,
        Err(e) => {
            eprintln!("PARSE ERROR: {}", e);
            std::process::exit(1);
        ...
        match lisp_rlm_wasm::program::run_program(&[expr.clone()], &mut env, &mut state) {
            Ok(v) => {
                ...
                eprintln!(
                    ...
                    i,
                    preview,
                    truncate_display(&v, 50)
                );
            }
            Err(e) => {
                fail += 1;
                eprintln!("  [FAIL] {:3}: {}", i, preview);
                eprintln!("          Error: {}", truncate_str(&e, 120));
            }
        }
    }
    eprintln!();
...
fn verify_file(path: &str) {
    ...
    eprintln!(
        "{} forms parsed, {} with expectations",
        exprs.len(),
        expected_count
    );
    if expectations.len() != exprs.len() {
        eprintln!(
            "WARNING: form tracker found {} forms, parser found {}",
            expectations.len(),
            exprs.len()
        );
    ...
    if checked > 0 {
        eprintln!("  Accuracy: {:.1}%", pass as f64 / checked as f64 * 100.0);
    }
