;; Ed25519 Wallet Contract
;; Storage layout:
;;   "pk"    → 32 bytes Ed25519 public key
;;   "nonce" → 4 bytes u32 LE (execution counter)
;;   "sk" + session_pk(32 bytes) → 4 bytes u32 LE (expiry block height)
...
(define (w_add_session_key)
  (let ((input (near/input)))
    (let ((session_pk (str-slice input 0 32))
          (expiry (bytes-to-u32 (str-slice input 32 36))))
      (near/store-bytes (str-cat "sk" session_pk) (u32-to-bytes expiry)))))

(define (w_execute_session)
  (let ((input (near/input)))
    (let ((session_pk (str-slice input 0 32))
          (nonce (bytes-to-u32 (str-slice input 32 36)))
          (sig_end (str-len input))
          (signed_data (str-slice input 36 (- sig_end 64)))
          (sig (str-slice input (- sig_end 64) sig_end)))
      (let ((stored_nonce (bytes-to-u32 (near/load-bytes "nonce"))))
        (if (!= nonce stored_nonce)
          (near/panic "bad nonce")
          (if (!= (near/ed25519_verify sig (str-cat (u32-to-bytes nonce) signed_data) session_pk) 1)
            (near/panic "bad sig")
            (begin
