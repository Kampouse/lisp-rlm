...
steps = [
    ...
    ("propose", dict({"name": "evented", "pexp": str(EXPIRES), "am": "10000000000000000000000",
                      "rc": "rich.test.near"},
                     **ev("propose:evented:0", 5100)), "ok"),
    ("approve", {"name": "evented", "id": "0", "ix": "0", "pubkey_hex": OPK,
                 "signature": apr_sig(APR1, "evented", "0", "0"),
    ...
    ("execute", {"name": "evented", "id": "0", "signature": owner_sig("execute:evented:0", 41),
                 "expires_at": str(EXPIRES), "nonce": "5101"}, "ERR_EV_REQUIRED"),
    ("execute", dict({"name": "evented", "id": "0"}, **ev("execute:evented:0", 5102)),
     "ERR_NOT_APPROVED"),
    ("approve", {"name": "evented", "id": "0", "ix": "0", "pubkey_hex": APR1_PK,
                 "signature": apr_sig(APR1, "evented", "0", "0"),
                 "expires_at": str(EXPIRES)}, "ok"),
    ("execute", dict({"name": "evented", "id": "0"}, **ev("execute:evented:0", 5103)),
     "ERR_NOT_APPROVED"),
    ("approve", {"name": "evented", "id": "0", "ix": "1", "pubkey_hex": APR2_PK,
                 "signature": apr_sig(APR2, "evented", "0", "1"),
                 "expires_at": str(EXPIRES)}, "ok"),
    ("execute", dict({"name": "evented", "id": "0"}, **ev("execute:evented:0", 5104)), "ok"),
    # already executed → not approved-for-execution
    ("execute", dict({"name": "evented", "id": "0"}, **ev("execute:evented:0", 5105)),
     "ERR_NOT_APPROVED"),
    ...
    ("propose", dict({"name": "evented", "pexp": str(EXPIRES), "am": "10",
                     ...
                     **ev("propose:evented:1", 5106)), "ok"),
    ("approve", {"name": "evented", "id": "1", "ix": "0", "pubkey_hex": APR1_PK,
                 "signature": apr_sig(APR1, "evented", "1", "0"),
                 "expires_at": str(EXPIRES)}, "ok"),
    ("approve", {"name": "evented", "id": "1", "ix": "1", "pubkey_hex": APR2_PK,
                 "signature": apr_sig(APR2, "evented", "1", "1"),
                 "expires_at": str(EXPIRES)}, "ok"),
    ("execute", dict({"name": "evented", "id": "1"}, **ev("execute:evented:1", 5107)),
    ...
    ("propose", dict({"name": "evented", "act": "appr", "np": APR2_PK, "nt": "1",
                      "pexp": str(EXPIRES)},
                     **ev("propose:evented:2", 5108)), "ok"),
    ("approve", {"name": "evented", "id": "2", "ix": "0", "pubkey_hex": APR1_PK,
                 "signature": apr_sig(APR1, "evented", "2", "0"),
                 "expires_at": str(EXPIRES)}, "ok"),
    ("execute", dict({"name": "evented", "id": "2"}, **ev("execute:evented:2", 5109)),
     "ERR_NOT_APPROVED"),
    ("approve", {"name": "evented", "id": "2", "ix": "1", "pubkey_hex": APR2_PK,
                 "signature": apr_sig(APR2, "evented", "2", "1"),
                 "expires_at": str(EXPIRES)}, "ok"),
    ("execute", dict({"name": "evented", "id": "2"}, **ev("execute:evented:2", 5110)), "ok"),
    ("get_approvers", {"name": "evented"}, f'{{"thr":"1","pks":"{APR2_PK}"}}'),
    ("propose", dict({"name": "evented", "act": "appr", "np": "", "nt": "1",
                      "pexp": str(EXPIRES)},
                     **ev("propose:evented:3", 5111)), "ERR_APPROVERS_EMPTY"),
    ("propose", dict({"name": "evented", "act": "appr", "np": f"{APR1_PK},{APR2_PK}",
                      "nt": "3", "pexp": str(EXPIRES)},
                     **ev("propose:evented:3", 5112)), "ERR_THRESHOLD_INVALID"),
    ("propose", dict({"name": "evented", "act": "zzz", "pexp": str(EXPIRES)},
                     **ev("propose:evented:3", 5113)), "ERR_ACTION_UNKNOWN"),
    ...
    ("propose", dict({"name": "evented", "pexp": str(EXPIRES), "am": "1",
                      "rc": "rich.test.near"},
                     **ev("propose:evented:3", 5116)), "ERR_PAUSED"),
    # unpause proposals only exist on the governance wallet
    ("propose", dict({"name": "evented", "act": "unp", "pexp": str(EXPIRES)},
                     **ev("propose:evented:3", 5117)), "ERR_NOT_GOVERNANCE"),
    # recovery path: propose unp on gov (bootstrap admin set = PK, thr 1)
    ("propose", dict({"name": "gov", "act": "unp", "pexp": str(EXPIRES)},
                     **ev("propose:gov:0", 5118)), "ok"),
    ("approve", {"name": "gov", "id": "0", "ix": "0", "pubkey_hex": PK,
                 "signature": apr_sig(SK, "gov", "0", "0"),
                 "expires_at": str(EXPIRES)}, "ok"),
    ("execute", dict({"name": "gov", "id": "0"}, **ev("execute:gov:0", 5119)), "ok"),
    ("is_paused", {}, "0"),

    # ── 7. admin-set rotation = self-governance on "gov" ───────────
    ("propose", dict({"name": "gov", "act": "appr", "np": OPK, "nt": "1",
                      "pexp": str(EXPIRES)},
                     **ev("propose:gov:1", 5120)), "ok"),
    ("approve", {"name": "gov", "id": "1", "ix": "0", "pubkey_hex": PK,
                 "signature": apr_sig(SK, "gov", "1", "0"),
                 "expires_at": str(EXPIRES)}, "ok"),
    ("execute", dict({"name": "gov", "id": "1"}, **ev("execute:gov:1", 5121)), "ok"),
    ...
    ("is_paused", {}, "1"),
    ("propose", dict({"name": "gov", "act": "unp", "pexp": str(EXPIRES)},
                     **ev("propose:gov:2", 5125, sk=OSK, pk=OPK)), "ok"),
    ("approve", {"name": "gov", "id": "2", "ix": "0", "pubkey_hex": OPK,
                 "signature": apr_sig(OSK, "gov", "2", "0"),
                 "expires_at": str(EXPIRES)}, "ok"),
    ("execute", dict({"name": "gov", "id": "2"}, **ev("execute:gov:2", 5126, sk=OSK, pk=OPK)),
     "ok"),
    ("is_paused", {}, "0"),
    ("get_version", {}, "2"),
]
